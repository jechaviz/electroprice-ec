...
const NotificationBell: React.FC = () => {
    ...
    return (
        <div className="relative">
            ...
            >
                <i className="fa-solid fa-bell text-xl text-base-content/70 group-hover:text-primary transition-colors"></i>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {notificationsSignal.value.length > 0 ? (
                            notificationsSignal.value.map(n => (
                                <div 
                                    key={n.id} 
                                    onClick={() => services.notification.markAsRead(n.id)}
                                    className={`p-4 border-b border-base-content/5 cursor-pointer hover:bg-base-300 transition-colors ${!n.read ? 'bg-primary/5 border-l-4 border-l-primary' : ''}`}
                                >
                            ...
                            <div className="p-12 text-center">
                                <i className="fa-solid fa-bell-slash text-4xl text-base-content/10 mb-4"></i>
                                <p className="text-xs font-bold text-base-content/30 uppercase tracking-widest">No new alerts</p>
                            </div>
                        )}
                    </div>

                    <div className="p-4 bg-base-300/50 text-center">
                        <p className="text-[10px] font-bold text-base-content/40 uppercase tracking-widest italic">Encrypted Connection</p>
                    </div>
                </div>
            )}
        </div>
